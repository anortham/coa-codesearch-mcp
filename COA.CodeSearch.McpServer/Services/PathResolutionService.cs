...
public class PathResolutionService : IPathResolutionService
{
    ...
    /// </summary>
    public void EnsureGitIgnoreExists()
    {
        try
        {
            ...
            {
                var gitignoreContent = @"# Ignore all local CodeSearch indexes and logs
# These are workspace-specific and can be regenerated by running index_workspace
...
!.gitignore
";
                Directory.CreateDirectory(basePath); // Ensure parent directory exists
                File.WriteAllText(gitignorePath, gitignoreContent);
                _logger.LogInformation("Created .gitignore in {Path} to prevent committing indexes", basePath);
            }
        }
        catch (Exception ex)
        {
            // Log error but don't fail startup if .gitignore creation fails
            _logger.LogWarning(ex, "Failed to create .gitignore in .coa/codesearch directory");
        }
    }
