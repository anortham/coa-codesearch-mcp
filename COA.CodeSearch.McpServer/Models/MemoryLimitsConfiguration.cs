...
/// <summary>
/// Memory pressure levels for controlling system behavior
/// </summary>
public enum MemoryPressureLevel
{
    /// <summary>Normal operation - no restrictions</summary>
    Normal = 0,
    
    /// <summary>Moderate pressure - reduce batch sizes and concurrency</summary>
    Moderate = 1,
    
    /// <summary>High pressure - significant throttling and cleanup</summary>
    High = 2,
    
    /// <summary>Critical pressure - emergency mode, minimal operations only</summary>
    Critical = 3
}

/// <summary>
/// Service for monitoring and controlling memory usage
/// </summary>
public interface IMemoryPressureService
{
    ...
    /// <summary>
    /// Check if operation should be throttled due to memory pressure
    /// </summary>
    bool ShouldThrottleOperation(string operationType);
    
    /// <summary>
    /// Get recommended batch size based on current memory pressure
    /// </summary>
    int GetRecommendedBatchSize(int maxBatchSize);
    
    /// <summary>
    /// Get recommended concurrency level based on current memory pressure
    /// </summary>
    int GetRecommendedConcurrency(int maxConcurrency);
    
    /// <summary>
    /// Force garbage collection if memory pressure is high
    /// </summary>
    Task TriggerMemoryCleanupIfNeededAsync();
}